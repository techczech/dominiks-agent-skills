...
It is a dependency-free reference implementation you can read end to end in one
sitting, then replace with a real compiler once the content model is settled.

No sample corpus ships with the skill. Create one or more source folders under
`examples/sources/` in the skill folder using the layout below, then run, from
the skill folder:

```bash
node scripts/build-sample-websites.mjs
```
