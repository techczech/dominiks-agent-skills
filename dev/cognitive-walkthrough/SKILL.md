...
description: "Run a structured cognitive walkthrough on any UI artefact — generated mockup, screenshot, live URL, paper prototype, Figma export — to evaluate learnability from a new user's perspective. Walks the user's task step by step, answers the four Nielsen Norman diagnostic questions per step, and writes a Markdown report with pass/fail verdicts and concrete fixes. Trigger this whenever the user asks for a cognitive walkthrough, learnability review, UX inspection, expert critique of a UI image, evaluation of a prototype, design review from a user's perspective, or wants to know whether a new user could actually complete a task on this interface — even when they don't use those exact words. Especially relevant after a new mockup is generated (image-to-design) and before the next revision, or when an interface is novel enough that established usability heuristics will miss its specific failures."
---

# Cognitive Walkthrough

Run a cognitive walkthrough on a UI artefact. The method is the Nielsen Norman Group's, not this skill's: pick a task, step through it as a new user, and at each step answer four diagnostic questions. Any *No* fails the step. The output is a short Markdown report with a verdict per step and a concrete fix where one is implied. Source: NN/g, "Cognitive Walkthroughs", <https://www.nngroup.com/articles/cognitive-walkthroughs/>. Credit it in any report you write; the template already does.

This skill is design-agnostic. The artefact can be a generated mockup from `image-to-design`, a screenshot, a deployed URL, a Figma export, or a paper prototype photographed and dropped into the conversation.
...
- After `image-to-design` generates a new version and before the next prompt revision.
...
2. **Design brief.** When the artefact lives inside an `image-to-design` folder, read `design-plans/<slug>/02-design-brief.md` and use the *Primary user task* field. If multiple tasks are listed, pick the one the user implied or ask which.
...
- **Inside an `image-to-design` folder:** write to `design-plans/<slug>/versions/vN/walkthroughs/<task-slug>.md`. Create the `walkthroughs/` directory if missing. The artefact context is already in the parent folder, so the report file is self-contained with frontmatter links upward.
...
When the user invokes this skill on an artefact inside a `design-plans/<slug>/versions/vN/` folder, follow the recipe in `references/integration-image-to-design.md`. Short version:
...
4. If the report fails one or more steps, suggest concrete prompt changes the user could feed into the next image-to-design revision (vN+1).
