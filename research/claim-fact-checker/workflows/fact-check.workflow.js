export const meta = {
  ...
  description: 'Fan-out fact-check: extract every non-obvious factual claim from a deliverable, then run one isolated verifier per claim against the archived sources plus a source-quality skeptic, and synthesize a findings + fix-plan pair. Works across research reports, paper reviews, and curated news items.',
  phases: [
...
const DOMAIN_FRAMING = {
  'research-report': 'The deliverable is a research report written from primary sources archived alongside it. Verdicts and output format follow the rubric in references/verdicts.md. Also sweep semantically for prescription-detected sentences (the report must present facts, never recommend action to an actor).',
