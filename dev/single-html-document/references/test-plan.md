...
- Run `node scripts/audit-single-html.mjs <file> --strict`.
- Run `node scripts/test-feedback-media-runtime.mjs` when annotation, feedback, or edit-diff export helpers are used.
...
These apply when you keep a sample corpus under the skill's own
`examples/sources/` folder. Run from the skill folder:

```bash
node scripts/verify-sample-websites.mjs
```

That rebuilds every sample, runs the static checks, and strict-audits each
packaged file. To inspect the results in a browser as well:
...
Then open `http://127.0.0.1:4173/<slug>/<slug>.single.html` for each sample slug
and repeat the browser checks above. Verify from `file://` too — the HTTP server
is a convenience, not the delivery condition.
