...
Usage:
    python3 clean-text.py <input_txt> <output_md> [--pdf-path <path>] [--page-count <n>] [--engine <name>]
...
def parse_args():
    ...
    parser.add_argument("--engine", default="poppler",
                        help="Engine name recorded in frontmatter (default: poppler)")
...
def add_frontmatter(text, pdf_path="", page_count=0, engine="poppler"):
...
def clean_text(raw_text, pdf_path="", page_count=0, engine="poppler"):
    ...
    # Add frontmatter
    text = add_frontmatter(text, pdf_path, page_count, engine)
...
def main():
    ...
    result = clean_text(raw_text, args.pdf_path, args.page_count, args.engine)
