...
const sourceDir = path.join(skillRoot, "examples/sources/qa-workbook");
const outputDir = path.join(skillRoot, "examples/generated/qa-workbook");
...
async function main() {
  ...
  console.log(`  normal: ${path.relative(skillRoot, path.join(siteDir, "index.html"))}`);
  console.log(`  single: ${path.relative(skillRoot, path.join(outputDir, "qa-workbook.single.html"))}`);
}
...
async function buildPlan(site) {
  ...
  return {
    ...
    sourcePath: path.relative(skillRoot, planPath),
...
function buildRuntime() {
  ...
  function clearDraft() {
    if (!window.confirm("Clear the saved draft for this workbook in this browser?")) return;
    state = emptyState();
