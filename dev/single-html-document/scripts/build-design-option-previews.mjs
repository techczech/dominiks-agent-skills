...
// `assets/design-samples` is the canonical home for the generated design
// samples; they ship with the skill as visual reference.
const skillDesignSamplesDir = path.join(skillRoot, "assets/design-samples");
const outputDirs = [skillDesignSamplesDir];
...
async function buildOutput(outDir) {
  ...
  for (const option of options) {
    ...
    console.log(`built ${path.relative(skillRoot, target)} (${formatBytes(size)})`);
  }
  if (outDir === skillDesignSamplesDir) {
    ...
    for (const sample of blockSamples) {
      ...
      console.log(`built ${path.relative(skillRoot, target)} (${formatBytes(result.sizeBytes)})`);
    }
  ...
  console.log(`built index (${path.relative(skillRoot, path.join(outDir, "index.html"))})`);
}
...
function readmeFor(outDir) {
  const rel = path.relative(skillRoot, outDir);
