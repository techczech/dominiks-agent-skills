...
One rubric for every preset, so a verdict means the same thing whichever kind of deliverable is being checked. Verdicts are products of **reading and reasoning, not keyword matching** — "semantics over grep". A verifier must build a model of what a claim asserts, read the source that would support it, and judge whether the source actually asserts the same thing.
...
3. Prescriptions (research-report domain)
4. Unsourced claims
