...
<!--
This file, and its sibling example-figure-01.md / example-table-01.md, show the
output of the `poppler` engine, whose heading and caption detection is purely
rule-based. The `docling`, `fast` and `math` engines produce the same directory
layout and the same `engine:` frontmatter key, but their Markdown body comes
from the engine's own converter rather than the heuristics described below.
-->
