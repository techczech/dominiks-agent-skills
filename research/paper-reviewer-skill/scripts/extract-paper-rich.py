...
Prerequisites:
  - academic-pdf-to-mkd skill (https://github.com/techczech/academic-pdf-to-mkd)
    cloned and `uv sync`-ed. Point ACADEMIC_PDF_SKILL at the clone; without it
    this script looks in ~/skills/academic-pdf-to-mkd.
...
# Where the companion academic-pdf-to-mkd clone lives. Override with the
# ACADEMIC_PDF_SKILL environment variable; the default is only a convention.
DEFAULT_ACADEMIC_PDF_SKILL = Path.home() / "skills" / "academic-pdf-to-mkd"
ACADEMIC_PDF_SKILL = Path(
    os.environ.get("ACADEMIC_PDF_SKILL") or DEFAULT_ACADEMIC_PDF_SKILL
).expanduser()
...
def run_extractor(pdf: Path, out_dir: Path, engine: str) -> None:
    if not EXTRACT_SH.is_file():
        sys.exit(
            f"error: academic-pdf-to-mkd extract-pdf.sh not found at {EXTRACT_SH}\n"
            "  Clone https://github.com/techczech/academic-pdf-to-mkd, run `uv sync`\n"
            "  in it, then set ACADEMIC_PDF_SKILL to that directory."
        )
