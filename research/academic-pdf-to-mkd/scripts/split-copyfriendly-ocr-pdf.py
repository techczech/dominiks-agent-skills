...
def draw_half_page(
    pdf: canvas.Canvas,
    image_path: Path,
    image_crop_box: tuple[int, int, int, int],
    lines: list[dict[str, float | str]],
    page_width: float,
    page_height: float,
    x_offset: float,
    ...
    with Image.open(image_path) as image:
        cropped = image.crop(image_crop_box)
        with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as temp_image:
            cropped.save(temp_image.name)
            cropped_path = Path(temp_image.name)

    try:
        draw_image_page(pdf, cropped_path, page_width, page_height)
    finally:
...
def line_right(line: dict[str, float | str]) -> float:
    return float(line["left"]) + float(line["width"])


def split_crop_for_lines(
    lines: list[dict[str, float | str]],
    opposite_lines: list[dict[str, float | str]],
    side: str,
    split_x: float,
    page_width: float,
    margin: float,
    ...
    if side == "left":
        content_edge = max((line_right(line) for line in lines), default=split_x)
        crop_right = max(split_x, content_edge + margin)
        if opposite_lines:
            opposite_edge = min(float(line["left"]) for line in opposite_lines)
            if opposite_edge < split_x:
                crop_right = max(content_edge + margin, opposite_edge - margin)
            else:
                crop_right = min(crop_right, max(content_edge + margin, opposite_edge - margin))
        return 0, min(page_width, max(content_edge, crop_right))

    content_edge = min((float(line["left"]) for line in lines), default=split_x)
    crop_left = min(split_x, content_edge - margin)
    if opposite_lines:
        opposite_edge = max(line_right(line) for line in opposite_lines)
        if content_edge >= split_x and opposite_edge > split_x:
            crop_left = max(split_x, min(content_edge - margin, opposite_edge + margin))
    return max(0, min(content_edge, crop_left)), page_width


def pdf_crop_to_image_crop(
    pdf_crop: tuple[float, float],
    page_width: float,
    image_width: int,
    image_height: int,
) -> tuple[int, int, int, int]:
    left, right = pdf_crop
    image_left = max(0, int(round((left / page_width) * image_width)))
    image_right = min(image_width, int(round((right / page_width) * image_width)))
    return (image_left, 0, image_right, image_height)
...
def build_pdf(source_pdf: Path, output_pdf: Path, split_spreads: bool) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="copyfriendly-ocr-") as temp_name:
        ...
        for index, (image_path, (page_width, page_height), lines) in enumerate(
            ...
            left_crop = split_crop_for_lines(
                left_lines,
                right_lines,
                "left",
                split_x,
            ...
            right_crop = split_crop_for_lines(
                right_lines,
                left_lines,
                "right",
                split_x,
            ...
            draw_half_page(
                pdf,
                image_path,
                pdf_crop_to_image_crop(left_crop, page_width, image_width, image_height),
                left_lines,
                left_crop[1] - left_crop[0],
                page_height,
                left_crop[0],
            )
            written_pages += 1
            ...
            if right_lines:
                draw_half_page(
                    pdf,
                    image_path,
                    pdf_crop_to_image_crop(right_crop, page_width, image_width, image_height),
                    right_lines,
                    right_crop[1] - right_crop[0],
                    page_height,
                    right_crop[0],
                )
                written_pages += 1
