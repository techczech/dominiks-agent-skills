...
Complete guide to importing academic papers from Zotero into the repository via a
VS Code export step.

> **Two import paths.** This one assumes you export from Zotero into a
> `source-files/` folder first, using the third-party **VSCodeZotero** extension,
> and then file each export by hand. It is the older path, and it is the right one
> when you already keep exports in the repo or want manual control over every
> paper folder. The default path is
> [workflow-zotero-direct.md](workflow-zotero-direct.md), where
> `scripts/zotero-import.py` queries your running Zotero over Better BibTeX's
> JSON-RPC and does all of this in one command. Start there unless you have a
> reason not to.
...
- **VSCodeZotero** extension installed in VS Code and configured to export into the
  host repo's `source-files/` folder
...
```bash
ls source-files/
# Look for subdirectories (e.g., llm-evaluation-studies/)
```

#### 2. List Papers in Collection

```bash
ls source-files/llm-evaluation-studies/*.md
```
...
papers:
  JZUBTJM9:
    ...
    collection_source: "source-files/llm-evaluation-studies/"
```

#### 5. Create Collection Index (Optional but Recommended)

Create `index/collections/llm-evaluation-studies.md`:

```markdown
---
type: collection
collection_name: "llm-evaluation-studies"
paper_count: 27
created: "2026-02-11"
...
# LLM Evaluation Studies Collection
...
```bash
git add source-files/llm-evaluation-studies/
git add papers/
git add index/
git commit -m "Import collection: llm-evaluation-studies (27 papers)"
```
