...
```bash
# 1. Clone this skill anywhere you keep skills; SKILL_DIR is used below
SKILL_DIR="$HOME/skills/paper-reviewer-skill"
git clone https://github.com/techczech/paper-reviewer-skill.git "$SKILL_DIR"
...
# 3. Link scripts/ and templates/ into the host repo so the CLI workflow
#    works without prefixing the skill path each time
...
Companion-skill dependency: clone [`academic-pdf-to-mkd`](https://github.com/techczech/academic-pdf-to-mkd) (a separate skill repo), `uv sync` it, and point `ACADEMIC_PDF_SKILL` at the clone. `extract-paper-rich.py` reads that environment variable and falls back to `~/skills/academic-pdf-to-mkd`.
...
- `generate-narration.py` — speaks a JSON narration script (segment text + delivery instruction per segment) into a WAV/MP3 using a local MLX TTS model. You write the narration; the script performs it. `--write-example` starts a skeleton. Keep scripts and audio under `papers/{folder}/narrations/`. Apple silicon + `ffmpeg`.
...
- **[`academic-pdf-to-mkd`](https://github.com/techczech/academic-pdf-to-mkd) skill**, cloned and `uv sync`-ed, with `ACADEMIC_PDF_SKILL` pointing at it (for rich extraction)
...
Generalised in 2026 from a single research repository that used these workflows in place, and from the earlier [`academic-paper-review`](https://github.com/techczech/academic-paper-review) skill, which it supersedes. That project is now simply one host of this skill; the skill is the durable, reusable layer.

Several reference files (`setup-architecture.md`, `directory-structure.md`, `workflow-import.md`, `workflow-extract.md`) still describe the predecessor's layout, in which the skill lived in a `skill/` folder at the host-repo root and PDFs were kept in a separate `source-files/` tree. Both layouts work. This SKILL.md describes the current one: the skill is cloned separately and each paper folder holds its own `00-source-{folder}.pdf`.
