...
Archived working note. It records how one family of deliverables — catalogue-style
output generated from a folder of structured Markdown records, with filters and
per-record share pages — was to be raised to reusable quality. The distilled
version of this thinking is in `references/design-options.md`; keep this file for
the component list and icon map, which are still a good starting inventory.

The plan follows the same ordering as the rest of the skill: content first,
normal site first, packaged single HTML last.
...
7. Add audit gates.
   ...
   ```bash
   node scripts/audit-single-html.mjs <file> --strict
   ```
