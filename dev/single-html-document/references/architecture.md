...
Two patterns from working projects support this ordering:

- Slide-deck-to-handout builders that render a normal React site first and treat media extraction as a second, separable stage.
- Content builds where Markdown/YAML is compiled into generated typed data before any component renders it.
...
- Vite gives fast builds, relative-path output, and straightforward asset inlining, which is what single-file packaging needs.
...
- the target file should stay comfortably inside the 10-30 MB range that email, chat, and shared drives handle without complaint
...
```bash
node scripts/make-attachment-manifest.mjs \
