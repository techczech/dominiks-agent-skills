...
> **Which layout is this?** This file documents the *vendored* layout, in which a
> copy of the skill lives in the host repo's own `skill/` folder and is committed
> alongside the research. It is the layout the predecessor skill
> [`academic-paper-review`](https://github.com/techczech/academic-paper-review)
> used, and it still works. The layout described in [SKILL.md](../SKILL.md) is
> different: the skill is cloned once, outside the host repo, and symlinked in.
> Choose the vendored layout when you want the workflow frozen and versioned with
> one specific research project; choose the SKILL.md layout when several repos
> should share one copy of the skill. The rest of this file assumes the vendored
> layout.
...
**Vendored approach**: Skill in project root, symlinked by all agents (transparent, accessible, shared)

## Directory Structure

```
{host-repo}/                              # Project root
...
├── .claude/
│   └── skills/
│       └── paper-reviewer-skill@ → ../../skill/      # Claude Code discovery
│
├── .antigravity/
│   └── skills/
│       └── paper-reviewer-skill@ → ../../skill/      # Antigravity discovery
│
├── .gemini/
│   └── skills/
│       └── paper-reviewer-skill@ → ../../skill/      # Gemini discovery
│
├── .codex/
│   └── skills/
│       └── paper-reviewer-skill@ → ../../skill/      # Codex discovery
│
...
### Vendoring the skill into a host repo

Run these from the host repo's root. Replace `paper-reviewer-skill` with whatever
name you want the agents to discover the skill under.
...
cd .claude/skills && ln -s ../../skill paper-reviewer-skill && cd ../..
cd .antigravity/skills && ln -s ../../skill paper-reviewer-skill && cd ../..
cd .gemini/skills && ln -s ../../skill paper-reviewer-skill && cd ../..
cd .codex/skills && ln -s ../../skill paper-reviewer-skill && cd ../..
...
Prefer the shared-clone layout in [SKILL.md](../SKILL.md#install-into-a-host-repo):
one clone of this repo, symlinked into as many host repos as you like. It gives
you the same agent discovery without committing a second copy of the workflow
...
ls -la .claude/skills/paper-reviewer-skill
# Should show: → ../../skill

ls -la .antigravity/skills/paper-reviewer-skill
# Should show: → ../../skill

ls -la .gemini/skills/paper-reviewer-skill
# Should show: → ../../skill

ls -la .codex/skills/paper-reviewer-skill
# Should show: → ../../skill
...
.claude/skills/paper-reviewer-skill@ → ../../skill/
.antigravity/skills/paper-reviewer-skill@ → ../../skill/
