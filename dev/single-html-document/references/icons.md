...
Run `node -e "import('./scripts/icons.mjs').then(m => console.log(m.ICON_NAMES.join(\\"\\n\\")))"` to dump the canonical list.

## Adding More Icons

The catalogue is intentionally small at the start so you only carry what's used. To add an icon:

1. Find it on [Lucide](https://lucide.dev/icons). Lucide is ISC-licensed, and the ISC notice is carried in the header of `scripts/icons.mjs` alongside the path data. Keep that notice in place when you copy the catalogue into a project.
