...
# A quoted empty YAML scalar. Hoisted out of the f-strings below because a
# backslash inside an f-string expression is a syntax error before Python 3.12.
EMPTY_YAML = '""'
...
def write_source_md(folder: Path, item: dict, pdf_rel: str, zotero_storage: str | None, folder_name: str) -> Path:
    ...
    fm_lines += [
        ...
        f"publication_title: {render_yaml_inline(item.get('publicationTitle', '')) or EMPTY_YAML}",
        f"volume: {render_yaml_inline(item.get('volume', '')) or EMPTY_YAML}",
        f"issue: {render_yaml_inline(item.get('issue', '')) or EMPTY_YAML}",
        f"pages: {render_yaml_inline(item.get('pages', '')) or EMPTY_YAML}",
        f"publisher: {render_yaml_inline(item.get('publisher', '')) or EMPTY_YAML}",
        f"language: {item.get('language', 'en')}",
        f"doi: {render_yaml_inline(item.get('DOI', '')) or EMPTY_YAML}",
        f"issn: {render_yaml_inline(item.get('ISSN', '')) or EMPTY_YAML}",
        f"url: {item.get('url', '')}",
