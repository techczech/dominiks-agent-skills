...
The frontmatter is machine-readable; treat field names as a contract. Tools that consume reports (the next iteration of image-to-design, a comparison viewer, an audit script) rely on the exact keys below.
...
links:
  ...
  manifest: "{{Optional: relative path to manifest.yml if image-to-design}}"
