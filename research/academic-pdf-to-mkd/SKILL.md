...
description: "Convert academic PDFs — papers, book chapters, scanned articles — into structured Markdown with figures, tables, and page images as separate files. Picks an extraction engine per document (Docling, pymupdf4llm, MinerU, Poppler) and OCRs scan-only PDFs first. Also repairs scanned PDFs into searchable, copy-friendly PDFs for reference managers."
---
...
|---|---|---|
| **docling** *(default for most papers)* | Two-column papers, layout-heavy PDFs, fall-through case | MIT. Runs on whatever accelerator PyTorch finds (MPS, CUDA, else CPU). Best figures/tables. |
...
These four were chosen from a wider survey of eight extractors — Docling, Marker,
MinerU, pymupdf4llm, markitdown, Nougat, olmOCR and Poppler — on two criteria:
licence terms permissive enough to embed, and structure recovery on two-column
academic layout. Per-engine licence terms are in [Licence notes](#licence-notes)
below; if you need a different trade-off, the dispatch table in
`scripts/extract-pdf.sh` is where another engine would be wired in.

## Prerequisites

Python deps via [`uv`](https://docs.astral.sh/uv/) (per `pyproject.toml`), run
from the skill directory:

```bash
cd path/to/academic-pdf-to-mkd
uv sync
```

This creates `.venv/` alongside `pyproject.toml` with `docling`, `pymupdf4llm`,
and `mineru[all]`. `extract-pdf.sh` and `ocr-pdf-for-zotero.sh` use
`.venv/bin/python` automatically when that directory exists and fall back to
`python3` otherwise. The poppler-branch helpers (`clean-text.py`,
`detect-tables.py`) import nothing outside the standard library and always run
under plain `python3`, which is why that engine works with no venv at all.

The committed `uv.lock` pins one resolution snapshot, so `uv sync` reproduces a
known-good dependency set rather than today's releases. Run `uv lock --upgrade`
if you want current versions instead.

System tools (macOS/Homebrew shown; use the equivalent packages on Linux):

```bash
brew install poppler ocrmypdf ghostscript
```

`poppler` supplies `pdftotext`, `pdftoppm`, `pdfinfo` and `pdfimages`, which
every branch of the pipeline uses. `ocrmypdf` is required for the OCR preflight
on scan-only PDFs. `ghostscript` is optional and only compresses the repaired
PDFs from the reference-manager branch. Missing optional tools are detected and
...
Reclaim the space later with
`rm -rf "${HF_HOME:-$HOME/.cache/huggingface}"/hub/models--opendatalab--*`.
...
Every engine writes an `engine:` key into the frontmatter of the fulltext and of
each figure/table file, and that key is what downstream steps should branch on.
It matters because table handling differs: `docling` and `poppler` write separate
files under `tables/`, while `fast` and `math` leave tables inline in the fulltext
Markdown. Only `docling` writes a CSV alongside each table.

`examples/` holds sample output from the `poppler` engine. The directory layout
and frontmatter keys are the same for every engine; the body text is not.

## Zotero-ready OCR PDF repair

This branch handles a different success criterion from Markdown extraction: a
reference manager needs a PDF that is searchable, selectable, and logically
paged. The script is named for Zotero because that is the common case, but the
output is a plain PDF — nothing in it is Zotero-specific, and it works the same
for Mendeley, Papers, DEVONthink or a bare filesystem.

### When to use

- Scan-only PDF: `pdftotext input.pdf - | wc -w` returns fewer than ~100 words.
- The user wants to file the PDF in a reference manager and search it later.
...
5. **Hand off:** Report the fulltext path, the engine used, and the figure/table counts. If a downstream reading or review workflow expects the fulltext in its own directory, move `{paper_id}-fulltext.md` there — the extraction step does not assume one.
