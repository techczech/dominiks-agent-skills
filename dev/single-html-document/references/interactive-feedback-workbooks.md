...
Use the commentable-workbook variant when the returned file is meant to be read
back by an agent or script: reviewer name, overall note, a drawer that opens by
default, a small selection CTA, one canonical local-storage state, Copy for
chat/agent, Copy JSON, Download JSON, and Clear all with confirmation.
...
```bash
node scripts/extract-feedback-media.mjs feedback.json --out feedback-media
```
