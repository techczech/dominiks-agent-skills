...
Nine options are catalogued below. Packaged samples for each live in
`assets/design-samples/`; `assets/design-samples/index.html` is a single-file
inspection gallery of the whole set. Run
`node scripts/build-design-option-previews.mjs` from the skill folder to
regenerate them.

Three lighter starter shapes recur across the options and are worth naming
separately, because most short deliverables are one of them:
...
Treat every sample as a direction, not a finished maximum-quality template.
...
Inspired by single-file conference and training decks that carry presenter
notes, screenshots, and an appendix inside the same HTML file.
...
This is the full version of the `gallery-report` starter shape.
...
Inspired by component library and pattern-handbook deliverables, including the patterns demonstrated in the [html-effectiveness examples](https://thariqs.github.io/html-effectiveness/).
...
## How The Starter Shapes Relate To The Options

The three lighter starter shapes are not separate options. They are the small-report
versions of options already listed:
...
Reach for a starter shape when the deliverable is short and the full option would
be over-built. The nine options are a menu, not a ladder.

## Raising The Quality Of A Record-Driven Catalogue

Catalogue-style output — single HTML generated from a folder of structured records,
with filters, per-record share pages, and meeting-friendly variants — is one of the
strongest uses of this skill. The core idea is sound; the quality gap is usually in
