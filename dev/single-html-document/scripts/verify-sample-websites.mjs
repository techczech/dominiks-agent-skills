...
const generatedRoot = path.join(skillRoot, "examples/generated");

async function main() {
  run("node", [path.join(scriptDir, "build-sample-websites.mjs")]);
  const manifest = JSON.parse(await readFile(path.join(generatedRoot, "manifest.json"), "utf8"));
  const results = [];

  for (const sample of manifest.samples) {
    const singlePath = path.resolve(skillRoot, sample.singleFile);
    ...
    run("node", [path.join(scriptDir, "audit-single-html.mjs"), sample.singleFile, "--strict"]);
    results.push({
      slug: sample.slug,
